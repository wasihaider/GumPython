...
class Choose(GumCommand):
    ...
    def cursor(self, cursor_text: str, prefix: str = None):
        ...
        self._add_to_flag_set(cursor_arg.cursor, get_string_input_object(cursor_text, cursor_arg.cursor.display_name))
        if prefix:
            self._add_to_flag_set(cursor_arg.prefix, get_string_input_object(prefix, cursor_arg.prefix.display_name))
        return self

    def cursor_align(self, alignment: str, margin: tuple = None, padding: tuple = None):
        cursor = self.arguments.cursor
        self._add_to_flag_set(cursor.align, get_alignment_object(alignment, cursor.align.display_name))
        if margin:
            self._add_to_flag_set(cursor.margin, get_position_object(margin, cursor.margin.display_name))
        if padding:
            self._add_to_flag_set(cursor.padding, get_position_object(padding, cursor.padding.display_name))
        return self

    def cursor_color(self, foreground_color: str, background_color: str = None):
        cursor = self.arguments.cursor
        self._add_to_flag_set(cursor.foreground, get_color_object(foreground_color))
        if background_color:
            self._add_to_flag_set(cursor.background, get_color_object(background_color))
        return self
    ...
    def cursor_border(self, style: str, foreground_color: str = None, background_color: str = None):
        ...
        if foreground_color:
            self._add_to_flag_set(cursor.border_foreground, get_color_object(foreground_color))
        if background_color:
            self._add_to_flag_set(cursor.border_background, get_color_object(background_color))
        return self

    def size(self, width: int = None, height: int = None):
        cursor = self.arguments.cursor
        if width:
            self._add_to_flag_set(cursor.width, get_integer_input_object(width, cursor.width.display_name))
        if height:
            self._add_to_flag_set(cursor.height, get_integer_input_object(height, cursor.height.display_name))
        return self
